import { IRepeaterFeeSlip } from "./types";
import { numberToWordsINR } from "./utiles";

function feeRes(data: any[]) {
  const date = new Date();
  ...
  if (!universityAddress)
    throw Error("Environment: UNIVERSITY_ADDRESS is not defined");
